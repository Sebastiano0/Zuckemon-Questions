...
let questions = []; // array vuoto per le domande
    ...
    <div id="additionalFields${questions.length}" class="hidden">
        <div>
            ...
            <input type="radio" name="soluzione${questions.length}" class="single-checkbox" id="soluzione1${questions.length}">
        </div>
        <div>
            <label for="risposta2${questions.length}">Risposta 2:</label>
            <input type="text" id="risposta2${questions.length}">
            <input type="radio" name="soluzione${questions.length}" class="single-checkbox" id="soluzione2${questions.length}">
        </div>
        <div>
            <label for="risposta3${questions.length}">Risposta 3:</label>
            <input type="text" id="risposta3${questions.length}">
            <input type="radio" name="soluzione${questions.length}" class="single-checkbox" id="soluzione3${questions.length}">
        </div>
        <div>
            <label for="risposta4${questions.length}">Risposta 4:</label>
            <input type="text" id="risposta4${questions.length}">
            <input type="radio" name="soluzione${questions.length}" class="single-checkbox" id="soluzione4${questions.length}">
        </div>
    </div>
                <div>
